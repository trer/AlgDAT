...
def longest_decreasing_subsequence(s):
    sequences = {}
    s = top_down_longest_decreasing_subsequence(s, 0, len(s), float('inf'), sequences)
    return s


def top_down_longest_decreasing_subsequence(s, i, n, prev, sequences):
    if (i, prev) in sequences:
        return sequences[(i, prev)]
    # Base case: not hing is remaining
    if i == n:
        return []
    # case 1: exclude the current element and process the
    # remaining elements
    excl = top_down_longest_decreasing_subsequence(s, i + 1, n, prev, sequences)

    # case 2: include the current element if it is smaller
    # than previous element in LDS
    incl = []
    if s[i] < prev:
        incl = [s[i]] + top_down_longest_decreasing_subsequence(s, i + 1, n, s[i], sequences)
    # return maximum of above two choices
    if len(incl) < len(excl):
        sequences[(i, prev)] = excl
        return excl
    sequences[(i, prev)] = incl
    return incl
