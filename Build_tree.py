...
def place_string_in_tree(root, dna):
    dna_list = list(dna)
    parent = root
    k=0
    for i in range(len(dna_list)):
        if not parent.children.__contains__(dna_list[i]):
            break
        k += 1
        parent = parent.children[dna_list[i]]
    if k == len(dna_list):
        parent.count += 1
    else:
        for j in range(k, len(dna_list)):
            parent.children.update({dna_list[j]: Node()})
            parent = parent.children[dna_list[j]]
        parent.count += 1
